 ...
 */
//adapted to xilinx interrupts by PBL
...
static XScuGic *pInterruptController = NULL;
...
xSemaphoreHandle startImuRecording;
StaticSemaphore_t startImuRecordingBuffer;
xSemaphoreHandle stopImuRecording;
StaticSemaphore_t stopImuRecordingBuffer;
xSemaphoreHandle imuRecordingFinished;
StaticSemaphore_t imuRecordingFinishedBuffer;
...
static void acc_data_ready_handler(void);

// logging
static void log_raw_data(Axis3i16 raw);
...
{
  systemWaitStart();
  DEBUG_PRINT("sensorsTask started\n");
  ...
  while (1)
  {
    if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY))
    {
      ...
      // data logging
      if (logEnabled ==  false) {
        if (pdTRUE == xSemaphoreTake(startImuRecording, 0)) {
          logEnabled = true;
          logBuffer = (int16_t*)IMU_LOGGING_DESTINATION_ADDR;
          logBufferSize = 0;
        }
      } else {
        if (pdTRUE == xSemaphoreTake(stopImuRecording, 0)) {
          logEnabled = false;
          xSemaphoreGive(imuRecordingFinished);
        }
      }
      ...
      if (logEnabled == true) {
        log_timestamp();
        log_raw_data(gyroRaw);
        log_raw_data(accelRaw);
      }
...
static void sensorsTaskInit(void)
{
  ...
  startImuRecording = xSemaphoreCreateBinaryStatic(&startImuRecordingBuffer);
  stopImuRecording = xSemaphoreCreateBinaryStatic(&stopImuRecordingBuffer);
  imuRecordingFinished = xSemaphoreCreateBinaryStatic(&imuRecordingFinishedBuffer);

  STATIC_MEM_TASK_CREATE(sensorsTask, sensorsTask, SENSORS_TASK_NAME, NULL, SENSORS_TASK_PRI);
}

void sensorsInterruptConfigure(XScuGic *pInterruptController2)
{
  pInterruptController = pInterruptController2;
}
void sensorsInterruptInit(void)
{
  ...
  if (pInterruptController == NULL)
  {
    DEBUG_PRINT("ERROR BMI088: no valid pInterruptController handle.\n");
    return;
  }
  ...
  /* highest priority, rising edge sensitive */
  XScuGic_SetPriorityTriggerType(pInterruptController, XPAR_FABRIC_IMU_INT1_INTR, 144, 0b11);

  /* interrupt handler IMU */ 
  /* Note: INT1 signal (accelerometer) is used to trigger reading of acceleromter and gyroscope */
  xStatus = XScuGic_Connect(pInterruptController, XPAR_FABRIC_IMU_INT1_INTR, (Xil_ExceptionHandler) acc_data_ready_handler, NULL);
  configASSERT(xStatus == XST_SUCCESS);
  (void) xStatus;

  /* enable interrupts */
  XScuGic_Enable(pInterruptController, XPAR_FABRIC_IMU_INT1_INTR);
}
...
static void sensorsBmi088Bmp3xxInit(void)
{
  ...
  sensorsDeviceInit();
  sensorsInterruptInit(); //note: first call sensorsInterruptConfigure in FreeRTOS_tick_config 
  sensorsTaskInit();
}
...
{
  char filename[32] = "imu";
  flash_write((uint8_t*)IMU_LOGGING_DESTINATION_ADDR, logBufferSize, filename);
}

static void log_raw_data(Axis3i16 raw)
{
  for (int i=0; i<3; i++) {
    *logBuffer = raw.axis[i];
    logBuffer++;
  }
  logBufferSize += 6;
}
...
static void log_timestamp(void)
{
  ...
  // read timestamp
  timestamp_lsb = Xil_In32(XPAR_IMU_RECORDER_0_BASEADDR + 8);
  timestamp_msb = Xil_In32(XPAR_IMU_RECORDER_0_BASEADDR + 12);

  // write to buffer
  *logBuffer = (timestamp_lsb >> 16) & 0xFFFF;
  logBuffer++;
  *logBuffer = timestamp_lsb & 0xFFFF;
  logBuffer++;
  *logBuffer = (timestamp_msb >> 16) & 0xFFFF;
  logBuffer++;
  *logBuffer = timestamp_msb & 0xFFFF;
  logBuffer++;
